...
function buyCoins_2(amount){ coins += amount; updateButtons(); updatePremiumPanel(); addNotification(`Bought ${amount} coins.`); }
setInterval(updatePremiumPanel, 5000); // refresh panel every 5s to reflect live changes
updatePremiumPanel();
// --- Live Premium Tier & Coin Panel ---
function updatePremiumPanel_2(){
  const panel_2 = document.getElementById('premium-panel');
  panel.innerHTML = `
    <h2>Upgrade / Buy Coins</h2>
    <p>Current Coins: ${coins} | Tier: ${tier}</p>
    <button onclick="buyCoins(100)">Buy 100 Coins</button>
...
function buyCoins_3(amount){ coins += amount; updateButtons(); updatePremiumPanel(); addNotification(`Bought ${amount} coins.`); }
setInterval(updatePremiumPanel, 5000); // refresh panel every 5s to reflect live changes
updatePremiumPanel();
// === Master Admin Panel ===
function showAdminPanel(){
  ...
  function refreshNodes(nodes){
    const tbody_2 = document.querySelector('#admin-nodes-table tbody');
    tbody.innerHTML='';
    nodes.forEach(n=>{
  ...
  function refreshClients(clients){
    const tbody_3 = document.querySelector('#admin-clients-table tbody');
    tbody.innerHTML='';
    clients.forEach(c=>{
...
function refreshCoinReferralStats_2(){
  socCoinsEarned.innerText=Math.floor(Math.random()*120);
  socNodeContributions.innerText=Math.floor(Math.random()*15)+1;
  socReferralsToday.innerText=Math.floor(Math.random()*5);
  socReferralBonus.innerText=(Math.floor(Math.random()*20));
...
function refreshNodes_2(nodes = []) {
  const tbody_4 = document.querySelector('#admin-nodes-table tbody'); tbody.innerHTML = '';
  nodes.forEach(node => { const row_8 = document.createElement('tr');
...
function deleteNode(id) {
    const tbody_5 = document.querySelector("#admin-nodes-table tbody");
    const nodeRow_2 = Array.from(tbody.rows).find(r => r.cells[0].innerText === id);
    if (nodeRow) {
...
wsMulti.onmessage = (event) => {
    try {
        const data_2 = JSON.parse(event.data);
...
async function safeUpdateCompliance(site) {
    const result_2 = await safeFetch(`/api/compliance?site=${encodeURIComponent(site)}`);
    if(result) {
...
wsMulti.onmessage = async (event) => {
    try {
        const data_4 = JSON.parse(event.data);
...
async function safeRefreshSOC() {
    try {
        const res_2 = await fetch("/api/soc");
        const data_5 = await res.json();
        socActiveThreats.innerText = data.activeThreats;
        socGlobalAttacks.innerText = data.globalAttacks;
        socActiveNodes.innerText = data.activeNodes;
        socSystemHealth.innerText = data.systemHealth;
...
async function safeRefreshCoins() {
    try {
        const res_3 = await fetch("/api/coins");
        const data_6 = await res.json();
        socCurrentCoins.innerText = data.currentCoins;
        socEstimatedCash.innerText = data.estimatedCash;
...
async function updateNodeBackend(id, newToken) {
    try {
        const res_4 = await fetch(`/api/nodes/${id}`, {
            method: "PUT",
...
async function deleteNodeBackend(id) {
    try {
        const res_5 = await fetch(`/api/nodes/${id}`, { method: "DELETE" });
...
function editNode_2(id) {
    const nodeRow_3 = Array.from(document.querySelectorAll("#admin-nodes-table tbody tr"))
        .find(r => r.cells[0].innerText === id);
    if(!nodeRow) return;
    const newToken_2 = prompt("Enter new token:", nodeRow.cells[1].innerText);
